...
##Plot folder
setwd(file.path(rootfolder,radio$plotDiffBind))
...
DBsample_simple <- dba.peakset(DBsample, consensus=c(DBA_TREATMENT), minOverlap=0.5)
...
df <- data.frame(seqnames=seqnames(report),
  ...
  write.table(df,file.path(rootfolder,radio$dataDiffBind,paste0(reportname,"_Occupancy.csv")),quote=F,row.names=F,col.names=F,sep="\t")
}
...
saveRDS(DBcount, file = file.path(rootfolder,radio$dataDiffBind,"dba.count.rds"))
...
colnames(df)[9] <- paste0(cond2,"_conc")
  ...
  write.table(df,file.path(rootfolder,radio$dataDiffBind,paste0(reportname,"_full.csv")),quote=F,col.names=T, row.names = F,sep="\t")
  ...
  write.table(df_select[,c(1,2,3,4)],file.path(rootfolder,radio$dataDiffBind,paste0(reportname,"_fdr-",FDR,"_fold-",fold,".csv")),quote=F,row.names=F,col.names=F,sep="\t")
